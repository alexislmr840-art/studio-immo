...
export default function ResultatsPage() {
  ...
  // FIX — maxLines respecté correctement, pas de débordement
  function wrapText(
    ctx: CanvasRenderingContext2D,
    text: string,
    x: number,
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (ctx.measureText(testLine).width > maxWidth && i > 0) {
        if (linesDrawn >= maxLines - 1) {
          // FIX — tronquer avec "…" sur la dernière ligne
  ...
  async function telechargerVisuel(
    ...
    try {
      ...
      wrapText(
        ctx,
        publication.accroche || "Votre futur bien vous attend",
        55,
        1100,
        970,
        62,
      ...
      // FIX — erreur de génération canvas catchée
      console.error("Erreur génération visuel :", error);
      alert("Erreur lors de la génération du visuel. Vérifiez que les photos sont bien chargées.");
    }
  }
              ...
              <section key={index} className="rounded-3xl bg-white p-6 shadow-xl">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-slate-600">{publication.objectif}</p>
                  </div>
                      ...
                      return (
                        <div key={key} className={`rounded-2xl ${bg} p-5`}>
                          <div className="flex items-center justify-between">
                            ...
                            {/* FIX — feedback "Copié !" sans alert bloquant */}
                            <button
                              onClick={() => copierTexte(publication[key], copyId)}
                              className={`rounded-xl px-4 py-2 font-bold ${btnBg}`}
                            >
                        ...
                        <div className="overflow-hidden rounded-2xl bg-white">
                            ...
                            <div className="flex items-center justify-between">
                              {logo ? (
                                <img
                                  src={logo}
                                  alt="Logo"
                                  className="h-10 max-w-28 rounded bg-white/90 object-contain p-1"
                                />
                              ) : (
                                <p className="font-bold text-white">
                                  {agence?.nomAgence || "Studio Immo"}
                                </p>
                              )}
                          ...
                          <div className="p-4 text-blue-950">
                            ...
                            <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold">
                              {bien?.ville && (
                                <span className="rounded-full bg-blue-950 px-3 py-1 text-white">
                                  📍 {bien.ville}
                                </span>
                              )}
                              {bien?.surface && (
                              ...
                              {bien?.afficherPrix && bien?.prix && (
                                <span className="rounded-full bg-amber-400 px-3 py-1">
                                  💰 {bien.prix}
                                </span>
                              )}
                            </div>
